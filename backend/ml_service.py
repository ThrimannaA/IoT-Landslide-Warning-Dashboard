...
def get_latest_readings(limit=50):
    ...
    try:
        url = f"{FIREBASE_DB_URL}/sensor_readings.json?orderBy=\"$key\"&limitToLast={limit}"
...
if __name__ == '__main__':
    print(f"\n{'='*60}")
    print(f"🚀 SiteSense ML Service Starting...")
    print(f"{'='*60}")
    print(f"📊 Firebase DB URL: {FIREBASE_DB_URL}")
    print(f"🤖 ML Model: {type(model).__name__ if model else 'Not loaded'}")
    print(f"📈 Features: {FEATURES}")
    print(f"🔮 Forecast Method: ML_model_on_predicted_features")
    print(f"\n✅ Ready! Available endpoints:")
    print(f"   GET  /api/ml/current-risk - Current risk using ML model")
    print(f"   GET  /api/ml/forecast?hours=X - ML-based forecast")
    print(f"   GET  /api/ml/historical-risk?limit=N - Historical ML predictions")
    print(f"   GET  /api/ml/feature-importance - Model explainability")
    print(f"   POST /api/ml/predict - Predict custom data")
    print(f"   GET  /api/ml/health - Health check")
    print(f"\n{'='*60}")
